...
import {
  LineChart, Line, BarChart, Bar, AreaChart, Area, XAxis, YAxis, Tooltip, ResponsiveContainer,
...
export default function TestAnalytics() {
  ...
  const minorTrend = latestMinor && minorTests.length > 1
    ? latestMinor.accuracy - minorTests[minorTests.length - 2].accuracy
    : 0
  const avgMinorPct = minorTests.length
    ? Math.round(minorTests.reduce((total, test) => total + test.accuracy, 0) / minorTests.length)
    : 0
  const minorChartData = minorTests.map((test, index) => ({
    name: `W${index + 1}`,
    label: test.name,
    Score: test.accuracy,
    Total: test.total,
  ...
  const handleAddMinor = () => {
    const maxMarks = Math.max(1, parseInt(newMinor.maxMarks) || 100)
    const physics = Math.max(0, parseInt(newMinor.physics) || 0)
    const chemistry = Math.max(0, parseInt(newMinor.chemistry) || 0)
    const maths = Math.max(0, parseInt(newMinor.maths) || 0)
    ...
    addMinorTest({
      ...
      total,
      maxMarks,
      accuracy: Math.round((total / maxMarks) * 100),
    })
    setNewMinor(EMPTY_MINOR)
    setShowAdd(false)
  }

  const TABS = ['overview', 'weekly', 'subject', 'trend', 'accuracy']
  ...
  return (
    <PageShell>
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
        ...
        {showAdd && (
          <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }}
            ...
            <div style={{ display: 'flex', gap: 8, marginBottom: 14 }}>
              {(['mock', 'minor'] as LogMode[]).map(mode => (
                <button key={mode} onClick={() => setLogMode(mode)}
                  className={logMode === mode ? 'btn-primary' : 'btn-ghost'}
                  style={{ fontSize: 11 }}>
                  {mode === 'mock' ? 'Mock Test (JEE)' : 'Weekly / Minor Test'}
                </button>
              ))}
            </div>
            {logMode === 'mock' ? (
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 10, marginBottom: 12 }}>
                ...
                <input placeholder="Physics (/100)" value={newTest.physics} onChange={e => setNewTest({ ...newTest, physics: e.target.value })} type="number" min={0} />
                <input placeholder="Chemistry (/100)" value={newTest.chemistry} onChange={e => setNewTest({ ...newTest, chemistry: e.target.value })} type="number" min={0} />
                <input placeholder="Maths (/100)" value={newTest.maths} onChange={e => setNewTest({ ...newTest, maths: e.target.value })} type="number" min={0} />
                <input placeholder="Time (min)" value={newTest.timeSpent} onChange={e => setNewTest({ ...newTest, timeSpent: e.target.value })} type="number" min={0} />
              </div>
            ) : (
              <>
                <div style={{ color: 'var(--text-muted)', fontSize: 12, marginBottom: 12 }}>Set a custom maximum score for school, coaching, or chapter tests.</div>
                <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr 1fr', gap: 10, marginBottom: 12 }}>
                  <input placeholder="Test name (e.g. Unit Test 1)" value={newMinor.name} onChange={e => setNewMinor({ ...newMinor, name: e.target.value })} />
                  <input placeholder="Max marks" value={newMinor.maxMarks} onChange={e => setNewMinor({ ...newMinor, maxMarks: e.target.value })} type="number" min={1} />
                  <input placeholder="Physics" value={newMinor.physics} onChange={e => setNewMinor({ ...newMinor, physics: e.target.value })} type="number" min={0} />
                  <input placeholder="Chemistry" value={newMinor.chemistry} onChange={e => setNewMinor({ ...newMinor, chemistry: e.target.value })} type="number" min={0} />
                  <input placeholder="Maths" value={newMinor.maths} onChange={e => setNewMinor({ ...newMinor, maths: e.target.value })} type="number" min={0} />
                </div>
                <div style={{ color: 'var(--amber)', fontSize: 12, marginBottom: 12, fontWeight: 700 }}>
                  Total: {(['physics', 'chemistry', 'maths'] as const).reduce((sum, key) => sum + (parseInt(newMinor[key]) || 0), 0)} / {parseInt(newMinor.maxMarks) || 100}
                  {' · '}{Math.round(((['physics', 'chemistry', 'maths'] as const).reduce((sum, key) => sum + (parseInt(newMinor[key]) || 0), 0) / (parseInt(newMinor.maxMarks) || 100)) * 100)}%
                </div>
              </>
            )}
            <div style={{ display: 'flex', gap: 8 }}>
              <button className="btn-primary" onClick={logMode === 'mock' ? handleAdd : handleAddMinor}>Save Test</button>
        ...
        {/* KPIs */}
        <div className="stat-grid analytics-kpis" style={{ marginBottom: 20 }}>
          {[
        ...
        <div className="card" style={{ padding: '14px 16px', marginBottom: 20, borderColor: '#ffaa0030' }}>
          <div className="section-title" style={{ color: 'var(--amber)', marginBottom: 6 }}>WEEKLY / MINOR TESTS</div>
          <div style={{ display: 'flex', alignItems: 'baseline', gap: 10 }}>
            <span className="stat-number" style={{ color: 'var(--amber)' }}>{latestMinor ? `${latestMinor.accuracy}%` : '—'}</span>
            <span style={{ color: 'var(--text-muted)', fontSize: 11 }}>
              {minorTests.length ? `${minorTests.length} logged · ${minorTrend >= 0 ? '+' : ''}${minorTrend}% trend · ${avgMinorPct}% average` : 'Log a custom-mark test to start its graph'}
            </span>
          </div>
        </div>
        ...
        {activeTab === 'weekly' && (
          <div className="card" style={{ padding: '18px 20px' }}>
            <div style={{ fontWeight: 700, marginBottom: 4 }}>Weekly / Minor Test Score Trend</div>
            <div style={{ color: 'var(--text-muted)', fontSize: 11, marginBottom: 14 }}>Percentage score based on each test's custom maximum marks.</div>
            {minorChartData.length === 0 ? (
              <div style={{ height: 260, display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--text-muted)', fontSize: 12 }}>No weekly tests logged yet.</div>
            ) : (
              <ResponsiveContainer width="100%" height={260}>
                <AreaChart data={minorChartData}>
                  <CartesianGrid stroke="#1a1a1a" strokeDasharray="3 3" />
                  <XAxis dataKey="label" tick={{ fill: '#666', fontSize: 10 }} />
                  <YAxis domain={[0, 100]} tick={{ fill: '#444', fontSize: 10 }} tickFormatter={value => `${value}%`} />
                  <Tooltip content={<CustomTooltip />} />
                  <ReferenceLine y={75} stroke="#39ff1444" strokeDasharray="4 4" />
                  <Area type="monotone" dataKey="Score" stroke="#ffaa00" fill="#ffaa0018" strokeWidth={2.5} />
                </AreaChart>
              </ResponsiveContainer>
            )}
          </div>
